...
public class DictionaryService {
    ...
    public void addMediaItem(@NotNull Dictionary dictionary,@NotNull MediaItem item) {
        String iName = item.getName();
        for (MediaItem i : dictionary.getMediaItems())
            if (i.getName().equals(iName))
                throw new ValidationException(String.format("Media Item  with name = %s already exist", iName));
